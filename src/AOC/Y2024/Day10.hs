...
partA g =
    let
        m = mapFromNestedLists g
        zs = indexZeroes g
     in
        foldl (\total z -> (+) total $ snd $ findAccesses empty m (Just z) 0) 0 zs
...
partB g =
    let
        ...
        zs = indexZeroes g
     in
        snd $ foldl (\(pathMap, total) z -> second (total +) $ findTrails pathMap m (Just z) 0) (empty, 0) zs
